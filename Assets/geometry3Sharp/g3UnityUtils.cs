...
public class g3UnityUtils
{
    ...
    public static GameObject CreateMeshGO(string name, DMesh3 mesh, Material setMaterial = null, bool bCollider = true)
    {
        ...
        if (setMaterial) {
            ...
            gameObj.AddComponent<MeshRenderer>().material = StandardMaterial(Colorf.Red);
        }
        return gameObj;
    }
    ...
    public static DMesh3 UnityMeshToDMesh(Mesh mesh)
    {
        ...
        for (int i = 0; i < mesh.vertexCount; ++i) {
            Vector3f _meshvertice = new Vector3f(mesh_vertices[i].x, mesh_vertices[i].y, mesh_vertices[i].z);
            dmesh_vertices[i] = _meshvertice;  //mesh_vertices[i];
        }
        ...
        if (mesh_normals != null) {
            ...
            {
                Vector3f _meshvertice = new Vector3f(mesh_vertices[i].x, mesh_vertices[i].y, mesh_vertices[i].z);
                dmesh_normals[i] = _meshvertice;  //mesh_normals[i];
            }
    ...
    public static void WriteOutputMesh(IMesh mesh, string sFilename, bool write_groups = true, bool write_vtxcolors = false, bool write_vtxuv = false)
    {
    ...
    public static DMesh3 LoadMesh(string sPath)
    {
